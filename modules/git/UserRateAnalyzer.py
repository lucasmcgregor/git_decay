    ...
    def analyze(self):
        ...
        for gfile in self.git_repo.get_all_files():
            for line in gfile.lines:
                if line is not None:
                    time_slice_date.add_author_data(line.date_added.replace(hour=1, minute=0, second=0, microsecond=0),
                                                    line.author, 1)
            for line in gfile.deleted_lines:
                if line is not None:
                    time_slice_date.add_author_data(
                        line.date_removed.replace(hour=1, minute=0, second=0, microsecond=0), line.remover, -1)
